#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/RPG/Client/TextID.h"
#include "unitysdk/System/Object.h"

class Class_1_7A22A3DBEEDD1F80;

#define RPG_GAMECORE_AVATARENHANCEDSKILLROW_FROMBINARY_OFFSET UNITYSDK_OFFSET(0x168A3270)
#define RPG_GAMECORE_AVATARENHANCEDSKILLROW__CTOR_OFFSET UNITYSDK_OFFSET(0x168A3780)

namespace RPG::GameCore
{
	inline static constexpr unsigned int AvatarEnhancedSkillRow_TypeDefinitionIndex = 11605;

	class AvatarEnhancedSkillRow : public ::System::Object
	{
	public:
		::System::UInt32 SkillTreeID; // 0x10
		::System::UInt32 AvatarID; // 0x14
		::RPG::Client::TextID DescAfter; // 0x18
		::RPG::Client::TextID SimpleDescAfter; // 0x28
		::System::UInt32 SkillID; // 0x38
		::System::UInt32 CommentIndex; // 0x3C
		::RPG::Client::TextID SimpleDescBefore; // 0x40
		::RPG::Client::TextID DescBefore; // 0x50
		::RPG::Client::TextID Comment02; // 0x60
		::RPG::Client::TextID Comment01; // 0x70

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_GAMECORE_AVATARENHANCEDSKILLROW__CTOR_OFFSET))(this);
		}

		static ::System::Void FromBinary(::Class_1_7A22A3DBEEDD1F80* array, ::RPG::GameCore::AvatarEnhancedSkillRow*& val)
		{
			return ((::System::Void(*)(::Class_1_7A22A3DBEEDD1F80*, ::RPG::GameCore::AvatarEnhancedSkillRow*&))((::PBYTE)hIl2Cpp + RPG_GAMECORE_AVATARENHANCEDSKILLROW_FROMBINARY_OFFSET))(array, val);
		}
	};
}
